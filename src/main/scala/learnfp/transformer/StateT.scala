...
object StateT {
  ...
  implicit def stateTMonadInstance[S, M[_]](implicit f:Functor[M], m:Monad[M]) = new Monad[({type E[X] = StateT[S, M, X]})#E] {
    ...
    override def flatMap[A, B](a: StateT[S, M, A])(fx: A => StateT[S, M, B]): StateT[S, M, B] = StateT { os =>
      a.runStateT(os) >>= { case (ns, na) =>
        fx(na).runStateT(ns)
      }
    }
  }
  ...
  def putT[S, M[_]](s:S)(implicit m:Monad[M]):StateT[S, M, Unit] = StateT {os:S => m.pure(s, ())}
  def getT[S, M[_]](implicit m:Monad[M]):StateT[S, M, S] = StateT {s:S => m.pure(s, s)}
}
